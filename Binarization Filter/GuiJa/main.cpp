...
void Controllers(HWND hWnd) {
    ...
    // Create thread count slider
    CreateWindowW(L"Static", L"Iloœæ w¹tków: ", WS_CHILD | WS_VISIBLE, 560, 708, 225, 25, hWnd, (HMENU)20, NULL, NULL);
    hThreadSlider = CreateWindowW(TRACKBAR_CLASSW, NULL, WS_CHILD | WS_VISIBLE | TBS_AUTOTICKS | TBS_TOOLTIPS,
        650, 705, 64, 25, hWnd, (HMENU)THREADS, NULL, NULL);
    SendMessageW(hThreadSlider, TBM_SETRANGE, TRUE, MAKELPARAM(1, 64));
    SendMessageW(hThreadSlider, TBM_SETPOS, TRUE, 4);
